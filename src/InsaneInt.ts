export class InsaneInt {
  ...
  isBalanced(): boolean {
    ...
    if (this.exponent > 0 && this.exponent < 1000000 && this.startingECount > 1)
      return false;
  ...
  add(other: InsaneInt) {
    ...
    if (myStartingECount > otherStartingECount) {
      otherExponent =
        otherExponent / Math.pow(10, myStartingECount - otherStartingECount);
      startingECount = myStartingECount;
    } else if (myStartingECount < otherStartingECount) {
      myExponent =
        myExponent / Math.pow(10, otherStartingECount - myStartingECount);
    ...
    if (myExponent > otherExponent) {
      coefficient =
        otherCoefficient / Math.pow(10, myExponent - otherExponent) +
        myCoefficient;
      exponent = myExponent;
    } else if (myExponent < otherExponent) {
      coefficient =
        myCoefficient / Math.pow(10, otherExponent - myExponent) +
        otherCoefficient;
  ...
  div(other: InsaneInt) {
    ...
    if (startingECount > other.startingECount) {
      exponent =
        exponent -
        other.exponent / Math.pow(10, startingECount - other.startingECount);
      startingECount = this.startingECount;
    } else if (startingECount < other.startingECount) {
      exponent =
        exponent / Math.pow(10, other.startingECount - startingECount) -
        other.exponent;
